...
Job state lives in the SQLite `jobs` table (see migrations), NOT in Streamlit

session state, so a browser refresh never loses or corrupts an indexing run.

This module provides a thin, typed API over the db job methods plus live stat
...
from .models import JobStatus, utcnow_iso
...
class JobManager:
    ...
    def active_job(self) -> Optional[Dict[str, Any]]:
        ...
        return jobs[0] if jobs else None✅ Part 1 of this batch complete — index_jobs.py is whole.Now the worker.PART: backlink_hunter_core/index_worker.pyPythonCopied!Copy"""Background streaming indexer.
