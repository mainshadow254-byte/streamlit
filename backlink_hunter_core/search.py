...
class SearchService:
    ...
    def _from_clause(self, f: SearchFilters) -> str:
        """Handle unique-source dedup via a grouped subquery when requested."""
        if f.unique_source_domain:
            return "(SELECT * FROM reverse_links GROUP BY source_domain) AS reverse_links"
        if f.unique_source_page:
            return "(SELECT * FROM reverse_links GROUP BY normalized_source_url) AS reverse_links"
    ...
    def page(self, f: SearchFilters, page: int = 1, page_size: int = 50) -> List[Dict[str, Any]]:
        where, params = self._build_where(f)
        frm = self._from_clause(f)
    ...
    def available_collections(self) -> List[str]:
        rows = self.db.conn.execute(
            "SELECT DISTINCT collection FROM reverse_links WHERE collection != '' ORDER BY collection"
        ).fetchall()
